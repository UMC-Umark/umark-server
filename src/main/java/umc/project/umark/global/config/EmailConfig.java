...
public class EmailConfig {
    @Value("${umark.mail.host}")
    private String emailHost;
    @Value(("${umark.mail.port}"))
    private int emailPort;
    @Value("${umark.mail.username}")
    private String emailUsername;
    @Value("${umark.mail.password}")
    ...
    public JavaMailSender mailSender() {//JAVA MAILSENDER 인터페이스를 구현한 객체를 빈으로 등록하기 위함.
        ...
        mailSender.setHost(emailHost);// 속성을 넣기 시작합니다. 이메일 전송에 사용할 SMTP 서버 호스트를 설정
        mailSender.setPort(emailPort);// 465로 포트를 지정
        mailSender.setUsername(emailUsername);//ID를 넣습니다.
        mailSender.setPassword(emailPassword);//비밀번호를 넣습니다.
